# The LogMonitor object is a thread that handles
...
class LogMonitor(Thread):
    ...
    def __init__(self,
        ...
        refreshPeriod: Period to check on the log in seconds
        alertThreshold: Number of hits/minute to trigger alarm
        monitorDuration: Data is only kept during this time in seconds
        """
    ...
    def read(self):
        ...
        try:
            ...
            self.stop("ERROR: LogMonitor cannot read the log file")
    ...
    def display_message(self):
        ...
        try:
            ...
            msg = "###############################\nWelcome to Arin's HTTP Monitor\n"
            msg +=    "###############################\n\n"
            ...
            self.pad.addstr("Summary statistics:\n", curses.A_BOLD)
            ...
            if self.alertStatus:
                ...
                self.pad.addstr("status: ALERT\n",
                                curses.color_pair(2))
            else:
                self.pad.addstr("                    ")
                curses.init_pair(3, curses.COLOR_GREEN, -1)
                self.pad.addstr("status: OK\n", curses.color_pair(3))
...
if __name__ == '__main__':
    ...
    threshold = float(config.get("Monitor", "threshold"))
    monitorDuration = float(config.get("Monitor", "monitorDuration"))
    logMonitor = LogMonitor(logPath, refreshPeriod, threshold, monitorDuration)
    logMonitor.start()
    logMonitor.join()
